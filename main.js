...
__export(main_exports, {
  default: () => GeminiFileGeneratorPlugin
});
...
var GeminiSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Max retry wait time (seconds)").setDesc("Maximum total wait time before giving up.").addText(
      (text) => text.setValue(String(this.plugin.settings.maxRetryWaitSec)).onChange(async (value) => {
        const v = Number(value);
        if (!Number.isNaN(v) && v > 0) {
...
function isRetryableError(err) {
  var _a2, _b, _c;
  const code = (err == null ? void 0 : err.status) || ((_a2 = err == null ? void 0 : err.error) == null ? void 0 : _a2.code) || ((_b = err == null ? void 0 : err.error) == null ? void 0 : _b.status) || ((_c = err == null ? void 0 : err.response) == null ? void 0 : _c.status);
...
function extractTextFromResponse(resp) {
  ...
  try {
    if (!resp)
      return null;
    if (typeof resp.text === "string" && resp.text.length)
      return resp.text;
    if (typeof resp.outputText === "string" && resp.outputText.length)
      return resp.outputText;
    const cand = (_a2 = resp.candidates) == null ? void 0 : _a2[0];
    if (((_b = cand == null ? void 0 : cand.content) == null ? void 0 : _b.parts) && Array.isArray(cand.content.parts)) {
      const parts = cand.content.parts.map((p) => (p == null ? void 0 : p.text) || "").filter((s2) => s2.length > 0);
      if (parts.length)
        return parts.join("\n\n");
    }
    if (Array.isArray(resp.output) && resp.output.length) {
      const texts = resp.output.map((o) => o.text || "").filter((s2) => s2.length);
      if (texts.length)
        return texts.join("\n\n");
    }
...
async function generateFromInlineData(apiKey, model, prompt, mimeType, base64Data, retryIntervalSec, maxRetryWaitSec) {
  ...
  const retryIntervalMs = retryIntervalSec * 1e3;
  const maxRetryWaitMs = maxRetryWaitSec * 1e3;
  let elapsedMs = 0;
  let lastError = null;
  let attempt = 0;
